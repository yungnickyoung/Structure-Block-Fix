package com.yungnickyoung.minecraft.structureblockfix.client.gui;
...
public class MegaStructureBlockEditScreen extends Screen {
    ...
    public MegaStructureBlockEditScreen(MegaStructureBlockEntity megaStructureBlockEntity) {
        ...
        this.structure = megaStructureBlockEntity;
    ...
    private void onDone() {
        if (this.sendToServer(MegaStructureBlockEntity.UpdateType.UPDATE_DATA)) {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new Button(this.width / 2 - 4 - 150, 210, 150, 20, CommonComponents.GUI_DONE, (button) -> {
            this.onDone();
        }));
        this.addRenderableWidget(new Button(this.width / 2 + 4, 210, 150, 20, CommonComponents.GUI_CANCEL, (button) -> {
            this.onCancel();
        }));
        ...
        this.addRenderableWidget(CycleButton
                .<StructureMode>builder((structureMode) -> new TranslatableComponent("structure_block.mode." + structureMode.getSerializedName()))
        ...
        this.includeEntitiesButton = this.addRenderableWidget(CycleButton
                .onOffBuilder(!this.structure.isIgnoreEntities())
                .displayOnlyValue()
                .create(this.width / 2 + 4 + 100, 160, 50, 20, INCLUDE_ENTITIES_LABEL, (cycleButton, bl) -> this.structure.setIgnoreEntities(!bl)));
        this.mirrorButton = this.addRenderableWidget(CycleButton
                .builder(Mirror::symbol)
        ...
        this.toggleAirButton = this.addRenderableWidget(CycleButton
                .onOffBuilder(this.structure.getShowAir())
                .displayOnlyValue()
                .create(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_AIR_LABEL, (cycleButton, boolean_) -> this.structure.setShowAir(boolean_)));
        this.toggleBoundingBox = this.addRenderableWidget(CycleButton
                .onOffBuilder(this.structure.getShowBoundingBox())
                .displayOnlyValue()
        ...
        this.nameEdit = new EditBox(this.font, this.width / 2 - 152, 40, 300, 20, new TranslatableComponent("structure_block.structure_name")) {
            public boolean charTyped(char c, int i) {
                return MegaStructureBlockEditScreen.this.isValidCharacterForName(this.getValue(), c, this.getCursorPosition()) && super.charTyped(c, i);
            }
        };
    ...
    private boolean sendToServer(MegaStructureBlockEntity.UpdateType updateType) {
        ...
        Services.NETWORK.sendSetMegaStructureBlockC2SPacket(this.structure.getBlockPos(), updateType, this.structure.getMode(),
                this.nameEdit.getValue(), blockPos, size, this.structure.getMirror(), this.structure.getRotation(),
                this.dataEdit.getValue(), this.structure.isIgnoreEntities(), this.structure.getShowAir(),
                this.structure.getShowBoundingBox(), integrity, seed);
        return true;
    }
